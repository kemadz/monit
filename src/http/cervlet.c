...
static void do_home(HttpRequest req, HttpResponse res) {
        ...
        StringBuffer_append(res->outputbuffer,
                            ...
                            "  <p align='center'>Monit is <a href='_runtime'>running</a> on %s and monitoring:</p>"
                            "  </td>"
                            " </tr>"
...
static char *get_service_status(Output_Type type, Service_T s, char *buf, int buflen) {
        ...
        if (s->monitor == Monitor_Not || s->monitor & Monitor_Init) {
                ...
                if (type == HTML)
                        snprintf(buf, buflen, "<span class='green-text'>%s</span>", s->type == 4 ? "Running" : statusnames[s->type]);
                else
                        snprintf(buf, buflen, Color_lightGreen("%s", s->type == 4 ? "Running" : statusnames[s->type]));
        } else {
